...
import { ArrowRight, ArrowLeft, Check, Lock, Home as HomeIcon, Phone, Mail, MapPin, LogOut, Plus, Trash2, X, RefreshCw, ClipboardCopy, ChevronDown, ChevronUp, Sparkles, ShieldCheck, Building2, KeyRound, } from "https://esm.sh/lucide-react@0.383.0?deps=react@18.3.1";
/* =========================================================================
...
.ihc-chip-light { font-size:12.5px; padding:7px 13px; border-radius:100px; border:1.5px solid var(--line); background: var(--sand); color: var(--ink-soft); cursor:pointer; }
.ihc-chip-light.on { background: var(--clay); border-color: var(--clay); color: var(--paper); }
...
const STYLE_CHIPS = ["Moderne", "Scandinave", "Industriel", "Classique", "Bohème", "Méditerranéen", "Minimaliste", "Éclectique", "Contemporain", "Rustique chic"];
const DISCOVERY_STEPS = [
    {
        title: "Coordonnées & cadrage du projet",
        subtitle: "Pour commencer, quelques repères sur le contexte.",
        fields: [
            ...
            { key: "bienStatus", label: "C'est un bien que vous possédez déjà, ou un achat en cours ?", type: "select", options: ["Déjà propriétaire", "Achat en cours / à venir"] },
            { key: "projetNature", label: "On parle d'une rénovation complète, de travaux ciblés, ou de décoration sans gros œuvre ?", type: "select", options: ["Rénovation complète", "Travaux ciblés", "Décoration sans gros œuvre"] },
            { key: "declencheur", label: "Qu'est-ce qui déclenche le projet maintenant ?", type: "textarea" },
            { key: "horizon", label: "Vous voyez ce logement comme un lieu de vie durable, ou une étape avant de revendre ?", type: "select", options: ["Projet de vie durable", "Étape avant revente", "Incertain"] },
        ],
    },
    {
        title: "Le foyer & le mode de vie",
        subtitle: "Comprendre le quotidien pour dessiner des espaces qui lui correspondent.",
        fields: [
            { key: "composition", label: "Qui vit (ou vivra) dans le logement ? Des enfants à venir ?", type: "textarea" },
            { key: "animaux", label: "Des animaux ?", type: "text" },
            { key: "teletravail", label: "Télétravail, besoin d'un bureau à la maison ?", type: "select", options: ["Oui", "Non", "Occasionnellement"] },
            { key: "reception", label: "Vous recevez souvent ? Plutôt intimiste ou grandes tablées ?", type: "textarea" },
            { key: "rythme", label: "Votre rythme au quotidien (matinaux, couche-tard, on cuisine beaucoup...) ?", type: "textarea" },
            { key: "besoinsParticuliers", label: "Des besoins particuliers à anticiper (mobilité, personnes âgées, jeunes enfants) ?", type: "textarea" },
        ],
    },
    {
        title: "Goûts & univers esthétique",
        subtitle: "Le moment de laisser parler les envies — supports visuels bienvenus en complément.",
        fields: [
            { key: "stylesAimes", label: "Quels styles vous font envie ?", type: "chips", options: STYLE_CHIPS },
            { key: "stylesExclus", label: "Un style ou une couleur que vous ne voulez surtout pas ?", type: "textarea" },
            { key: "inspirations", label: "Des lieux, photos ou ambiances qui vous font rêver ?", type: "textarea" },
            { key: "meublesAConserver", label: "Des meubles/objets à conserver absolument, ou à faire disparaître ?", type: "textarea" },
            { key: "rapportCouleurMatiere", label: "Couleurs affirmées ou palette neutre ? Matières brutes ou plus douces ?", type: "textarea" },
        ],
    },
    {
        title: "Usage des espaces & projets à venir",
        subtitle: "Ce qui fonctionne, ce qui coince, et ce qui pourrait changer demain.",
        fields: [
            { key: "frustrationsActuelles", label: "Qu'est-ce qui vous frustre le plus dans l'agencement actuel ?", type: "textarea" },
            { key: "rangement", label: "Le rangement est-il suffisant ? Qu'est-ce qui manque de place ?", type: "textarea" },
            { key: "agrandissementFamille", label: "Un agrandissement de la famille est-il envisagé ?", type: "select", options: ["Oui", "Non", "Peut-être"] },
            { key: "intentionRevente", label: "Une intention de revente à moyen terme ?", type: "select", options: ["Oui, à moyen terme", "Non, projet durable", "Incertain"] },
            { key: "phasage", label: "Vous préférez tout faire d'un coup, ou par étapes selon le budget ?", type: "select", options: ["Tout en une fois", "Par étapes selon le budget"] },
        ],
    },
    {
        title: "Cadre pratique & notes",
        subtitle: "On termine par le concret, avant de passer au chiffrage.",
        fields: [
            { key: "budgetEnveloppe", label: "Quelle enveloppe budgétaire avez-vous en tête ?", type: "text" },
            { key: "delaiSouhaite", label: "Un délai ou une échéance à respecter ?", type: "text" },
            { key: "decideurs", label: "Qui décide, et souhaitez-vous être associé(e) à chaque choix ?", type: "textarea" },
            { key: "implicationSouhaitee", label: "Niveau d'implication souhaité", type: "select", options: ["Être associé à chaque choix", "Nous faire confiance sur les détails"] },
            { key: "notesDecorateur", label: "Notes libres — impressions générales à chaud", type: "textarea" },
        ],
    },
];
...
async function sbUpdateLeadData(id, data) {
    try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_leads?id=eq.${id}`, {
            method: "PATCH",
            headers: sbHeaders({ Prefer: "return=minimal" }),
...
async function sbDeleteLeads(ids) {
    if (!ids || !ids.length)
        return true;
    try {
        const list = ids.map((id) => `"${id}"`).join(",");
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_leads?id=in.(${list})`, {
            method: "DELETE",
            headers: sbHeaders({ Prefer: "return=minimal" }),
        });
        return res.ok;
...
async function sbGetDiscoveries() {
    try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_discoveries?select=*&order=created_at.desc`, { headers: sbHeaders() });
        if (!res.ok)
            return [];
        const rows = await res.json();
        return rows.map((r) => ({ id: r.id, date: r.created_at, status: r.status, ...r.data }));
...
async function sbInsertDiscovery(data) {
    try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_discoveries`, {
            method: "POST",
            headers: sbHeaders({ Prefer: "return=representation" }),
            body: JSON.stringify({ status: "new", data }),
        });
        if (!res.ok)
            return null;
        const rows = await res.json();
...
async function sbUpdateDiscoveryData(id, data) {
    try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_discoveries?id=eq.${id}`, {
            method: "PATCH",
            headers: sbHeaders({ Prefer: "return=minimal" }),
            body: JSON.stringify({ data }),
        });
...
async function sbDeleteDiscoveries(ids) {
    if (!ids || !ids.length)
        return true;
    try {
        const list = ids.map((id) => `"${id}"`).join(",");
        const res = await fetch(`${SUPABASE_URL}/rest/v1/ihc_discoveries?id=in.(${list})`, {
            method: "DELETE",
            headers: sbHeaders({ Prefer: "return=minimal" }),
        });
        return res.ok;
...
export default function App() {
    ...
    const [discoveryOrigin, setDiscoveryOrigin] = useState("home"); // "home" | "admin"
    ...
    async function loadAll() {
        const [cfg, storedHash, leadsData, discoveriesData, metricsData] = await Promise.all([
    ...
    async function updateLeadNotes(lead, notes) {
        setLeads((prev) => prev.map((l) => (l.id === lead.id ? { ...l, notes } : l)));
        const { id, date, status, ...data } = lead;
        return sbUpdateLeadData(id, { ...data, notes });
    }
    async function archiveLeads(ids) {
        ...
        await Promise.all(targets.map((lead) => {
            const { id, date, status, ...data } = lead;
            return sbUpdateLeadData(id, { ...data, archived: true });
        }));
        flashToast(`${ids.length} demande(s) déplacée(s) vers les demandes supprimées`);
    }
    async function restoreLeads(ids) {
        ...
        await Promise.all(targets.map((lead) => {
            const { id, date, status, ...data } = lead;
            return sbUpdateLeadData(id, { ...data, archived: false });
        }));
        flashToast(`${ids.length} demande(s) restaurée(s)`);
    }
    async function permanentlyDeleteLeads(ids) {
        setLeads((prev) => prev.filter((l) => !ids.includes(l.id)));
        const ok = await sbDeleteLeads(ids);
        flashToast(ok ? `${ids.length} demande(s) définitivement supprimée(s)` : "Échec de la suppression — réessayez");
    }
    async function archiveDiscoveries(ids) {
        const targets = discoveries.filter((d) => ids.includes(d.id));
        setDiscoveries((prev) => prev.map((d) => (ids.includes(d.id) ? { ...d, archived: true } : d)));
        await Promise.all(targets.map((d) => {
            const { id, date, status, ...data } = d;
            return sbUpdateDiscoveryData(id, { ...data, archived: true });
        }));
        flashToast(`${ids.length} découverte(s) déplacée(s) vers les découvertes supprimées`);
    }
    async function restoreDiscoveries(ids) {
        const targets = discoveries.filter((d) => ids.includes(d.id));
        setDiscoveries((prev) => prev.map((d) => (ids.includes(d.id) ? { ...d, archived: false } : d)));
        await Promise.all(targets.map((d) => {
            const { id, date, status, ...data } = d;
            return sbUpdateDiscoveryData(id, { ...data, archived: false });
        }));
        flashToast(`${ids.length} découverte(s) restaurée(s)`);
    }
    async function permanentlyDeleteDiscoveries(ids) {
        setDiscoveries((prev) => prev.filter((d) => !ids.includes(d.id)));
        const ok = await sbDeleteDiscoveries(ids);
        flashToast(ok ? `${ids.length} découverte(s) définitivement supprimée(s)` : "Échec de la suppression — réessayez");
    }
    async function updateDiscoveryNotes(discovery, notes) {
        setDiscoveries((prev) => prev.map((d) => (d.id === discovery.id ? { ...d, notes } : d)));
        const { id, date, status, ...data } = discovery;
        return sbUpdateDiscoveryData(id, { ...data, notes });
    }
    ...
    function startDiscovery(origin = "home") {
        resetWizard();
    ...
    function updateDiscoveryAnswer(key, value) {
    ...
    function discoveryNext() {
        ...
        setDiscoveryStep((s) => clamp(s + 1, 0, DISCOVERY_STEPS.length - 1));
    }
    function discoveryPrev() {
        if (discoveryStep === 0) {
            setDiscoveryMode(false);
            setView(discoveryOrigin === "admin" ? "admin" : "home");
            return;
        }
        setDiscoveryStep((s) => clamp(s - 1, 0, DISCOVERY_STEPS.length - 1));
    }
    async function saveDiscovery() {
        ...
        const pieces = selectedPieceIds().map((id) => ({ id, ...pieceSel[id] }));
        const piecesLines = pieces.map((p) => {
        ...
        const data = {
            ...discoveryAnswers,
            habitationType, habitationLabel,
            tierId, tierLabel,
            piecesDetail: piecesLines,
            postesDetail: postesLines,
            low: estimate?.low || 0,
            high: estimate?.high || 0,
            notes: "",
        };
        const inserted = await sbInsertDiscovery(data);
        const record = inserted || { id: uid("disc"), date: new Date().toISOString(), status: "new", ...data };
        setDiscoveries((prev) => [record, ...prev]);
        setDiscoverySaved(true);
        flashToast(inserted ? "Découverte client enregistrée" : "Enregistrée localement — vérifiez la configuration Supabase");
    }
    ...
    return (React.createElement("div", { className: "ihc" },
        ...
        view !== "admin" && view !== "admin-login" && view !== "discovery" && view !== "discovery-result" && (React.createElement("div", { className: "ihc-topbar" },
        ...
        view === "home" && React.createElement(HomeView, { config: config, onStart: startSimulation, onStartDiscovery: () => startDiscovery("home") }),
        view === "discovery" && (React.createElement(DiscoveryView, { stepIndex: discoveryStep, answers: discoveryAnswers, onAnswer: updateDiscoveryAnswer, onNext: discoveryNext, onPrev: discoveryPrev })),
        view === "wizard" && (React.createElement(WizardView, { config: config, step: step, setStep: setStep, habitationType: habitationType, setHabitationType: setHabitationType, pieceSel: pieceSel, setPieceSel: setPieceSel, tierId: tierId, setTierId: setTierId, posteSel: posteSel, setPosteSel: setPosteSel, applicablePostes: applicablePostes, selectedPieceIds: selectedPieceIds, onFinish: goResult, onAbort: () => { if (discoveryMode) {
        ...
        view === "discovery-result" && estimate && (React.createElement(DiscoveryResultView, { estimate: estimate, answers: discoveryAnswers, saved: discoverySaved, onSave: saveDiscovery, onNewDiscovery: startDiscovery, onExit: () => { setDiscoveryMode(false); setView(discoveryOrigin === "admin" ? "admin" : "home"); } })),
        ...
        view === "admin" && (React.createElement(AdminDashboard, { config: config, setConfig: persistConfig, leads: leads, updateLeadStatus: updateLeadStatus, updateLeadNotes: updateLeadNotes, archiveLeads: archiveLeads, restoreLeads: restoreLeads, permanentlyDeleteLeads: permanentlyDeleteLeads, discoveries: discoveries, updateDiscoveryNotes: updateDiscoveryNotes, archiveDiscoveries: archiveDiscoveries, restoreDiscoveries: restoreDiscoveries, permanentlyDeleteDiscoveries: permanentlyDeleteDiscoveries, onStartDiscovery: () => startDiscovery("admin"), metrics: metrics, setPasswordHash: persistPasswordHash, onExit: () => setView("home"), flashToast: flashToast })),
        view !== "admin" && view !== "admin-login" && view !== "discovery" && view !== "discovery-result" && (React.createElement("div", { className: "ihc-shell" },
...
/* ================================ HOME VIEW =============================== */
function HomeView({ config, onStart, onStartDiscovery }) {
    return (React.createElement("div", { className: "ihc-shell" },
        React.createElement("div", { className: "ihc-hero" },
            ...
            React.createElement("div", { style: { display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" } },
                ...
                React.createElement("button", { className: "btn btn-ghost ihc-hero-cta-desktop", onClick: onStartDiscovery },
                    React.createElement(Sparkles, { size: 16 }),
                    " D\u00E9marrer une d\u00E9couverte client")),
            React.createElement("div", { style: { marginTop: 10, fontSize: 11.5, color: "var(--ink-soft)" }, className: "ihc-hero-cta-desktop" }, "R\u00E9serv\u00E9 aux \u00E9quipes ID'Home Cr\u00E9ation \u2014 \u00E0 utiliser en rendez-vous avec le client.")),
        React.createElement("div", { className: "ihc-mobile-cta-bar", style: { display: "flex", flexDirection: "column", gap: 8 } },
            React.createElement("button", { className: "btn btn-clay", onClick: onStart },
                "D\u00E9marrer la simulation ",
                React.createElement(ArrowRight, { size: 17 })),
            React.createElement("button", { className: "btn btn-ghost", onClick: onStartDiscovery },
                React.createElement(Sparkles, { size: 16 }),
                " D\u00E9couverte client"))));
}
/* ================================ DISCOVERY VIEW =========================== */
function DiscoveryField({ field, value, onChange }) {
    if (field.type === "text") {
        return React.createElement("input", { value: value || "", onChange: (e) => onChange(e.target.value) });
    }
    if (field.type === "textarea") {
        return React.createElement("textarea", { rows: 3, value: value || "", onChange: (e) => onChange(e.target.value) });
    }
    if (field.type === "select") {
        return (React.createElement("select", { value: value || "", onChange: (e) => onChange(e.target.value) },
            React.createElement("option", { value: "" }, "\u2014"),
            field.options.map((o) => React.createElement("option", { key: o, value: o }, o))));
    }
    if (field.type === "chips") {
        const selectedVals = Array.isArray(value) ? value : [];
        function toggle(opt) {
            onChange(selectedVals.includes(opt) ? selectedVals.filter((v) => v !== opt) : [...selectedVals, opt]);
        }
        return (React.createElement("div", { className: "ihc-chip-group", style: { marginTop: 4 } }, field.options.map((o) => (React.createElement("button", { type: "button", key: o, className: "ihc-chip-light" + (selectedVals.includes(o) ? " on" : ""), onClick: () => toggle(o) }, o)))));
    }
    return null;
}
function DiscoveryView({ stepIndex, answers, onAnswer, onNext, onPrev }) {
    const stepDef = DISCOVERY_STEPS[stepIndex];
    useEffect(() => {
        const el = document.getElementById("ihc-discovery-top");
        if (el)
            el.scrollIntoView({ behavior: "smooth", block: "start" });
    }, [stepIndex]);
    return (React.createElement("div", { className: "ihc-shell" },
        React.createElement("div", { style: { paddingTop: 8 }, id: "ihc-discovery-top" },
            React.createElement("span", { className: "ihc-eyebrow" },
                React.createElement(Sparkles, { size: 13 }),
                " D\u00E9couverte client \u2014 \u00E9tape ",
                stepIndex + 1,
                " / ",
                DISCOVERY_STEPS.length),
            React.createElement("div", { className: "ihc-card", style: { marginTop: 16 } },
                React.createElement("h2", { className: "ihc-q-title" }, stepDef.title),
                React.createElement("p", { className: "ihc-q-sub" }, stepDef.subtitle),
                stepDef.fields.map((f) => (React.createElement("div", { className: "ihc-field", key: f.key },
                    React.createElement("label", null, f.label),
                    React.createElement(DiscoveryField, { field: f, value: answers[f.key], onChange: (v) => onAnswer(f.key, v) }))))),
            React.createElement("div", { className: "ihc-nav-wrap" },
                React.createElement("div", { className: "ihc-nav" },
                    React.createElement("button", { className: "btn btn-ghost", onClick: onPrev },
                    ...
                    React.createElement("button", { className: "btn btn-primary", onClick: onNext },
                        stepIndex === DISCOVERY_STEPS.length - 1 ? "Passer au chiffrage" : "Suivant",
                        " ",
...
function DiscoveryResultView({ estimate, answers, saved, onSave, onNewDiscovery, onExit }) {
    return (React.createElement("div", { className: "ihc-shell" },
        React.createElement("div", { style: { paddingTop: 8 } },
            React.createElement("span", { className: "ihc-eyebrow" },
                React.createElement(ShieldCheck, { size: 13 }),
                " D\u00E9couverte client \u2014 synth\u00E8se"),
            React.createElement("div", { className: "ihc-card ihc-result-hero", style: { marginTop: 16 } },
                React.createElement("div", { className: "ihc-result-range mono" },
                    euro(estimate.low),
                    " \u2014 ",
                    euro(estimate.high)),
                React.createElement("p", { className: "ihc-result-note" }, "Estimation calcul\u00E9e \u00E0 partir des r\u00E9ponses de l'entretien. D\u00E9tail complet ci-dessous \u2014 rien n'est verrouill\u00E9, vous \u00EAtes avec le client."),
                React.createElement("div", { className: "ihc-breakdown" }, estimate.rows.map((r) => (React.createElement("div", { key: r.id, className: "ihc-bd-row" },
                    React.createElement("span", null, r.label),
                    React.createElement("span", { className: "mono" }, euro(r.lineTotal)))))),
                React.createElement("div", { style: { textAlign: "left", marginTop: 24, paddingTop: 20, borderTop: "1px dashed var(--line)" } },
                    React.createElement("div", { style: { fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--ink-soft)", marginBottom: 10 } }, "R\u00E9capitulatif de l'entretien"),
                    React.createElement("div", { style: { fontSize: 13.5, lineHeight: 1.7 } },
                        ...
                        answers.bienStatus && React.createElement(React.Fragment, null,
                            answers.bienStatus,
                            " \u00B7 ",
                            answers.projetNature,
                            React.createElement("br", null)),
                        answers.stylesAimes && Array.isArray(answers.stylesAimes) && answers.stylesAimes.length > 0 && React.createElement(React.Fragment, null,
                            "Styles : ",
                            answers.stylesAimes.join(", "),
                            React.createElement("br", null)),
                        answers.budgetEnveloppe && React.createElement(React.Fragment, null,
                            "Budget pressenti : ",
                            answers.budgetEnveloppe,
                            React.createElement("br", null)))),
                saved ? (React.createElement("div", { style: { marginTop: 22, fontSize: 13.5, color: "var(--garrigue-deep)", fontWeight: 700 } },
                    React.createElement(Check, { size: 15, style: { verticalAlign: -2 } }),
                    " D\u00E9couverte enregistr\u00E9e dans l'espace professionnel.")) : (React.createElement("button", { className: "btn btn-clay", style: { marginTop: 22 }, onClick: onSave },
                    "Enregistrer la d\u00E9couverte client ",
                    React.createElement(ArrowRight, { size: 16 }))),
                React.createElement("div", { style: { marginTop: 20, display: "flex", gap: 10, justifyContent: "center" } },
                    React.createElement("button", { className: "btn btn-ghost btn-sm", onClick: onNewDiscovery },
                        React.createElement(RefreshCw, { size: 14 }),
                        " Nouvelle d\u00E9couverte"),
                    React.createElement("button", { className: "btn btn-ghost btn-sm", onClick: onExit }, "Retour \u00E0 l'espace pro"))))));
}
/* ============================== LEAD FORM MODAL ============================ */
...
/* ================================ ADMIN DASHBOARD ============================ */
function AdminDashboard({ config, setConfig, leads, updateLeadStatus, updateLeadNotes, archiveLeads, restoreLeads, permanentlyDeleteLeads, discoveries, updateDiscoveryNotes, archiveDiscoveries, restoreDiscoveries, permanentlyDeleteDiscoveries, onStartDiscovery, metrics, setPasswordHash, onExit, flashToast }) {
    const [tab, setTab] = useState("apercu");
    const tabs = [
        { id: "apercu", label: "Aperçu" },
        { id: "discoveries", label: `Découvertes clients${discoveries.length ? " (" + discoveries.length + ")" : ""}` },
    ...
    return (React.createElement("div", { className: "ihc-admin" },
        React.createElement("div", { className: "ihc-admin-topbar" },
            ...
            React.createElement("div", { style: { display: "flex", gap: 10 } },
                React.createElement("button", { className: "btn btn-clay btn-sm", onClick: onStartDiscovery },
                    React.createElement(Sparkles, { size: 14 }),
                    " Nouvelle d\u00E9couverte client"),
        ...
        React.createElement("div", { className: "ihc-admin-shell" },
            ...
            tab === "discoveries" && React.createElement(AdminDiscoveries, { discoveries: discoveries, onUpdateNotes: updateDiscoveryNotes, onArchive: archiveDiscoveries, onRestore: restoreDiscoveries, onPermanentlyDelete: permanentlyDeleteDiscoveries, flashToast: flashToast }),
            ...
            tab === "leads" && React.createElement(AdminLeads, { leads: leads, onToggleStatus: updateLeadStatus, onUpdateNotes: updateLeadNotes, onArchive: archiveLeads, onRestore: restoreLeads, onPermanentlyDelete: permanentlyDeleteLeads, flashToast: flashToast }),
...
function LeadCard({ lead, selected, onToggleSelect, onToggleStatus, onUpdateNotes }) {
    ...
    function handleNotesChange(v) {
        ...
        timer.current = window.setTimeout(async () => {
            await onUpdateNotes(lead, v);
            setSaved(true);
        }, 800);
    }
    const sectionLabelStyle = { fontSize: 11, color: "rgba(239,231,214,0.45)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 };
    return (React.createElement("div", { className: "ihc-lead-card", style: selected ? { borderColor: "var(--clay)", background: "rgba(156,68,32,0.12)" } : undefined },
        React.createElement("div", { className: "ihc-lead-top" },
            React.createElement("div", { style: { display: "flex", gap: 12 } },
                React.createElement("input", { type: "checkbox", checked: selected, onChange: () => onToggleSelect(lead.id), "aria-label": `Sélectionner la demande de ${lead.prenom} ${lead.nom}`, style: { width: 18, height: 18, marginTop: 2, flexShrink: 0 } }),
                React.createElement("div", null,
                    ...
                    React.createElement("div", { className: "ihc-lead-meta" },
                        ...
                        lead.creneau,
                        React.createElement("br", null),
                        lead.habitationLabel || lead.habitationType,
                        " \u00B7 ",
                        lead.tierLabel || lead.tierId,
                        " \u00B7 Estimation : ",
                        euro(lead.low),
                        " \u2014 ",
                        euro(lead.high),
            ...
            React.createElement("button", { className: "ihc-status-badge " + (lead.status === "new" ? "new" : "done"), onClick: () => onToggleStatus(lead.id, lead.status) }, lead.status === "new" ? "Nouveau" : "Traité")),
        React.createElement("button", { type: "button", className: "btn btn-ghost btn-sm", style: { marginTop: 12, color: "var(--sand)", borderColor: "rgba(239,231,214,0.25)" }, onClick: () => setExpanded((e) => !e) },
            expanded ? React.createElement(ChevronUp, { size: 14 }) : React.createElement(ChevronDown, { size: 14 }),
            " ",
            expanded ? "Masquer le détail" : "Voir le détail & notes"),
        expanded && (React.createElement("div", { style: { marginTop: 14, paddingTop: 14, borderTop: "1px solid rgba(239,231,214,0.12)" } },
            React.createElement("div", { style: sectionLabelStyle }, "Pi\u00E8ces \u00E0 r\u00E9nover"),
            (lead.piecesDetail || []).length ? lead.piecesDetail.map((l, i) => (React.createElement("div", { key: i, style: { fontSize: 13, marginBottom: 3 } },
                "\u2022 ",
                l))) : React.createElement("div", { style: { fontSize: 13, color: "rgba(239,231,214,0.5)" } }, "Aucune donn\u00E9e"),
            React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Postes de travaux souhait\u00E9s"),
            (lead.postesDetail || []).length ? lead.postesDetail.map((l, i) => (React.createElement("div", { key: i, style: { fontSize: 13, marginBottom: 3 } },
                "\u2022 ",
                l))) : React.createElement("div", { style: { fontSize: 13, color: "rgba(239,231,214,0.5)" } }, "Aucune donn\u00E9e"),
            lead.message && (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Message du prospect"),
                React.createElement("div", { style: { fontSize: 13 } }, lead.message))),
            React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Notes internes"),
            React.createElement("textarea", { rows: 3, value: notes, onChange: (e) => handleNotesChange(e.target.value), placeholder: "Ex. Rappel\u00E9 le 24/08, int\u00E9ress\u00E9 mais budget serr\u00E9, relancer semaine prochaine\u2026", style: { width: "100%", background: "rgba(239,231,214,0.08)", border: "1px solid rgba(239,231,214,0.2)", color: "var(--sand)", borderRadius: 6, padding: "9px 11px", fontFamily: "inherit", fontSize: 13 } }),
            React.createElement("div", { style: { fontSize: 11, color: saved ? "rgba(239,231,214,0.4)" : "#e0c56f", marginTop: 5 } }, saved ? "Enregistré" : "Enregistrement…")))));
}
function AdminLeads({ leads, onToggleStatus, onUpdateNotes, onArchive, onRestore, onPermanentlyDelete, flashToast }) {
    ...
    const toggleSelectActive = toggle(setSelectedActive);
    const toggleSelectArchived = toggle(setSelectedArchived);
    async function handleArchiveSelected() {
        const ids = Array.from(selectedActive);
    ...
    async function confirmPermanentDeleteSelected() {
        const ids = Array.from(selectedArchived);
        setConfirmPermanentDelete(false);
        setSelectedArchived(new Set());
        await onPermanentlyDelete(ids);
    }
    function toCsv() {
        const header = ["Date", "Prénom", "Nom", "Téléphone", "Email", "Adresse", "Créneau", "Fourchette basse", "Fourchette haute", "Statut", "Notes"];
        const rows = leads.map((l) => [new Date(l.date).toLocaleString("fr-FR"), l.prenom, l.nom, l.telephone, l.email, l.adresse, l.creneau, l.low, l.high, l.status === "new" ? "Nouveau" : "Traité", l.notes || ""]);
    ...
    const linkBtnStyle = { background: "none", border: "none", color: "rgba(239,231,214,0.7)", fontSize: 12.5, textDecoration: "underline", textUnderlineOffset: "3px", padding: 0, cursor: "pointer" };
    if (!leads.length)
        return React.createElement("div", { className: "ihc-panel" },
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 } },
            React.createElement("div", { style: { display: "flex", gap: 14, alignItems: "center" } },
                activeLeads.length > 0 && (React.createElement("button", { type: "button", onClick: () => setSelectedActive(selectedActive.size === activeLeads.length ? new Set() : new Set(activeLeads.map((l) => l.id))), style: linkBtnStyle }, selectedActive.size === activeLeads.length ? "Tout désélectionner" : "Tout sélectionner")),
                selectedActive.size > 0 && React.createElement("span", { style: { fontSize: 12.5, color: "rgba(239,231,214,0.6)" } },
                    selectedActive.size,
                    " s\u00E9lectionn\u00E9e(s)")),
            React.createElement("div", { style: { display: "flex", gap: 10 } },
                selectedActive.size > 0 && (React.createElement("button", { className: "btn btn-danger btn-sm", onClick: handleArchiveSelected },
                    React.createElement(Trash2, { size: 14 }),
                    " Archiver (",
                    selectedActive.size,
                    ")")),
        ...
        activeLeads.length ? activeLeads.map((l) => (React.createElement(LeadCard, { key: l.id, lead: l, selected: selectedActive.has(l.id), onToggleSelect: toggleSelectActive, onToggleStatus: onToggleStatus, onUpdateNotes: onUpdateNotes }))) : React.createElement("div", { className: "ihc-empty" }, "Toutes les demandes ont \u00E9t\u00E9 archiv\u00E9es."),
        React.createElement("div", { style: { marginTop: 28, borderTop: "1px solid rgba(239,231,214,0.14)", paddingTop: 16 } },
            React.createElement("button", { type: "button", onClick: () => setArchivedOpen((o) => !o), style: { ...linkBtnStyle, display: "flex", alignItems: "center", gap: 6, textDecoration: "none", fontWeight: 700, fontSize: 13 } },
                archivedOpen ? React.createElement(ChevronUp, { size: 15 }) : React.createElement(ChevronDown, { size: 15 }),
                " Demandes supprim\u00E9es ",
                archivedLeads.length ? `(${archivedLeads.length})` : ""),
            archivedOpen && (React.createElement("div", { style: { marginTop: 14 } }, !archivedLeads.length ? (React.createElement("div", { className: "ihc-empty" }, "Aucune demande archiv\u00E9e.")) : (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 } },
                    React.createElement("div", { style: { display: "flex", gap: 14, alignItems: "center" } },
                        React.createElement("button", { type: "button", onClick: () => setSelectedArchived(selectedArchived.size === archivedLeads.length ? new Set() : new Set(archivedLeads.map((l) => l.id))), style: linkBtnStyle }, selectedArchived.size === archivedLeads.length ? "Tout désélectionner" : "Tout sélectionner"),
                        selectedArchived.size > 0 && React.createElement("span", { style: { fontSize: 12.5, color: "rgba(239,231,214,0.6)" } },
                            selectedArchived.size,
                            " s\u00E9lectionn\u00E9e(s)")),
                    selectedArchived.size > 0 && (React.createElement("div", { style: { display: "flex", gap: 10 } },
                        React.createElement("button", { className: "btn btn-ghost btn-sm", style: { color: "var(--sand)", borderColor: "rgba(239,231,214,0.25)" }, onClick: handleRestoreSelected },
                            React.createElement(RefreshCw, { size: 14 }),
                            " Restaurer (",
                            selectedArchived.size,
                            ")"),
                        React.createElement("button", { className: "btn btn-danger btn-sm", onClick: () => setConfirmPermanentDelete(true) },
                            React.createElement(Trash2, { size: 14 }),
                            " Supprimer d\u00E9finitivement (",
                            selectedArchived.size,
                            ")")))),
                archivedLeads.map((l) => (React.createElement(LeadCard, { key: l.id, lead: l, selected: selectedArchived.has(l.id), onToggleSelect: toggleSelectArchived, onToggleStatus: onToggleStatus, onUpdateNotes: onUpdateNotes })))))))),
        React.createElement(ConfirmDialog, { open: confirmPermanentDelete, title: `Supprimer définitivement ${selectedArchived.size} demande(s) ?`, body: "Cette action est irr\u00E9versible : les demandes et leurs notes internes seront perdues pour toujours.", onConfirm: confirmPermanentDeleteSelected, onCancel: () => setConfirmPermanentDelete(false) })));
}
function DiscoveryCard({ discovery: d, selected, onToggleSelect, onUpdateNotes }) {
    const [expanded, setExpanded] = useState(false);
    ...
    function handleNotesChange(v) {
        ...
        if (timer.current)
            window.clearTimeout(timer.current);
        timer.current = window.setTimeout(async () => {
            await onUpdateNotes(d, v);
            setSaved(true);
        }, 800);
    }
    const sectionLabelStyle = { fontSize: 11, color: "rgba(239,231,214,0.45)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 };
    const row = (label, value) => value ? (React.createElement("div", { style: { marginBottom: 10 } },
        React.createElement("div", { style: { ...sectionLabelStyle, marginBottom: 3 } }, label),
        React.createElement("div", { style: { fontSize: 13 } }, Array.isArray(value) ? value.join(", ") : value))) : null;
    return (React.createElement("div", { className: "ihc-lead-card", style: selected ? { borderColor: "var(--clay)", background: "rgba(156,68,32,0.12)" } : undefined },
        React.createElement("div", { className: "ihc-lead-top" },
            React.createElement("div", { style: { display: "flex", gap: 12 } },
                React.createElement("input", { type: "checkbox", checked: selected, onChange: () => onToggleSelect(d.id), "aria-label": `Sélectionner la découverte de ${d.prenom} ${d.nom}`, style: { width: 18, height: 18, marginTop: 2, flexShrink: 0 } }),
                React.createElement("div", null,
                    React.createElement("div", { className: "ihc-lead-name" },
                        d.prenom,
                        " ",
                    ...
                    React.createElement("div", { className: "ihc-lead-meta" },
                        ...
                        d.adresse || "—",
                        React.createElement("br", null),
                        d.habitationLabel || d.habitationType,
                        " \u00B7 ",
                        d.tierLabel || d.tierId,
                        " \u00B7 Estimation : ",
                        euro(d.low),
                        " \u2014 ",
                        euro(d.high),
        ...
        React.createElement("button", { type: "button", className: "btn btn-ghost btn-sm", style: { marginTop: 12, color: "var(--sand)", borderColor: "rgba(239,231,214,0.25)" }, onClick: () => setExpanded((e) => !e) },
            expanded ? React.createElement(ChevronUp, { size: 14 }) : React.createElement(ChevronDown, { size: 14 }),
            " ",
            expanded ? "Masquer le détail" : "Voir le détail de l'entretien"),
        expanded && (React.createElement("div", { style: { marginTop: 14, paddingTop: 14, borderTop: "1px solid rgba(239,231,214,0.12)" } },
            row("Statut du bien / nature du projet", [d.bienStatus, d.projetNature].filter(Boolean).join(" · ")),
            row("Déclencheur du projet", d.declencheur),
            row("Horizon", d.horizon),
            row("Composition du foyer", d.composition),
            row("Animaux", d.animaux),
            row("Télétravail", d.teletravail),
            row("Réception", d.reception),
            row("Rythme de vie", d.rythme),
            row("Besoins particuliers", d.besoinsParticuliers),
            row("Styles aimés", d.stylesAimes),
            row("Styles/couleurs exclus", d.stylesExclus),
            row("Inspirations", d.inspirations),
            row("Meubles à conserver / écarter", d.meublesAConserver),
            row("Rapport couleur / matière", d.rapportCouleurMatiere),
            row("Frustrations actuelles", d.frustrationsActuelles),
            row("Rangement", d.rangement),
            row("Agrandissement famille envisagé", d.agrandissementFamille),
            row("Intention de revente", d.intentionRevente),
            row("Phasage souhaité", d.phasage),
            row("Budget enveloppe pressenti", d.budgetEnveloppe),
            row("Délai souhaité", d.delaiSouhaite),
            row("Décideurs / implication", [d.decideurs, d.implicationSouhaitee].filter(Boolean).join(" · ")),
            row("Notes du décorateur (prises pendant l'entretien)", d.notesDecorateur),
            React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Pi\u00E8ces \u00E0 r\u00E9nover"),
            (d.piecesDetail || []).length ? d.piecesDetail.map((l, i) => React.createElement("div", { key: i, style: { fontSize: 13, marginBottom: 3 } },
                "\u2022 ",
                l)) : React.createElement("div", { style: { fontSize: 13, color: "rgba(239,231,214,0.5)" } }, "Aucune donn\u00E9e"),
            React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Postes de travaux chiffr\u00E9s"),
            (d.postesDetail || []).length ? d.postesDetail.map((l, i) => React.createElement("div", { key: i, style: { fontSize: 13, marginBottom: 3 } },
                "\u2022 ",
                l)) : React.createElement("div", { style: { fontSize: 13, color: "rgba(239,231,214,0.5)" } }, "Aucune donn\u00E9e"),
            React.createElement("div", { style: { ...sectionLabelStyle, marginTop: 16 } }, "Notes internes (suivi commercial)"),
            React.createElement("textarea", { rows: 3, value: notes, onChange: (e) => handleNotesChange(e.target.value), placeholder: "Ex. Devis envoy\u00E9 le 24/08, en attente de retour\u2026", style: { width: "100%", background: "rgba(239,231,214,0.08)", border: "1px solid rgba(239,231,214,0.2)", color: "var(--sand)", borderRadius: 6, padding: "9px 11px", fontFamily: "inherit", fontSize: 13 } }),
            React.createElement("div", { style: { fontSize: 11, color: saved ? "rgba(239,231,214,0.4)" : "#e0c56f", marginTop: 5 } }, saved ? "Enregistré" : "Enregistrement…")))));
}
function AdminDiscoveries({ discoveries, onUpdateNotes, onArchive, onRestore, onPermanentlyDelete, flashToast }) {
    const [selectedActive, setSelectedActive] = useState(() => new Set());
    const [selectedArchived, setSelectedArchived] = useState(() => new Set());
    const [archivedOpen, setArchivedOpen] = useState(false);
    const [confirmPermanentDelete, setConfirmPermanentDelete] = useState(false);
    const activeDiscoveries = discoveries.filter((d) => !d.archived);
    const archivedDiscoveries = discoveries.filter((d) => d.archived);
    function toggle(setter) {
        return (id) => setter((prev) => {
            const next = new Set(prev);
            if (next.has(id))
    ...
    if (!discoveries.length)
        return React.createElement("div", { className: "ihc-panel" },
            React.createElement("div", { className: "ihc-empty" }, "Aucune d\u00E9couverte client pour le moment. Cliquez \u00AB Nouvelle d\u00E9couverte client \u00BB en haut de l'\u00E9cran pour d\u00E9marrer un entretien."));
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 } },
            React.createElement("div", { style: { display: "flex", gap: 14, alignItems: "center" } },
                activeDiscoveries.length > 0 && (React.createElement("button", { type: "button", onClick: () => setSelectedActive(selectedActive.size === activeDiscoveries.length ? new Set() : new Set(activeDiscoveries.map((d) => d.id))), style: linkBtnStyle }, selectedActive.size === activeDiscoveries.length ? "Tout désélectionner" : "Tout sélectionner")),
                selectedActive.size > 0 && React.createElement("span", { style: { fontSize: 12.5, color: "rgba(239,231,214,0.6)" } },
                    selectedActive.size,
                    " s\u00E9lectionn\u00E9e(s)")),
            selectedActive.size > 0 && (React.createElement("button", { className: "btn btn-danger btn-sm", onClick: handleArchiveSelected },
                ...
                selectedActive.size,
                ")"))),
        activeDiscoveries.length ? activeDiscoveries.map((d) => (React.createElement(DiscoveryCard, { key: d.id, discovery: d, selected: selectedActive.has(d.id), onToggleSelect: toggleSelectActive, onUpdateNotes: onUpdateNotes }))) : React.createElement("div", { className: "ihc-empty" }, "Toutes les d\u00E9couvertes ont \u00E9t\u00E9 archiv\u00E9es."),
        React.createElement("div", { style: { marginTop: 28, borderTop: "1px solid rgba(239,231,214,0.14)", paddingTop: 16 } },
            React.createElement("button", { type: "button", onClick: () => setArchivedOpen((o) => !o), style: { ...linkBtnStyle, display: "flex", alignItems: "center", gap: 6, textDecoration: "none", fontWeight: 700, fontSize: 13 } },
                archivedOpen ? React.createElement(ChevronUp, { size: 15 }) : React.createElement(ChevronDown, { size: 15 }),
                " D\u00E9couvertes supprim\u00E9es ",
                archivedDiscoveries.length ? `(${archivedDiscoveries.length})` : ""),
            archivedOpen && (React.createElement("div", { style: { marginTop: 14 } }, !archivedDiscoveries.length ? (React.createElement("div", { className: "ihc-empty" }, "Aucune d\u00E9couverte archiv\u00E9e.")) : (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 } },
                    React.createElement("div", { style: { display: "flex", gap: 14, alignItems: "center" } },
                        React.createElement("button", { type: "button", onClick: () => setSelectedArchived(selectedArchived.size === archivedDiscoveries.length ? new Set() : new Set(archivedDiscoveries.map((d) => d.id))), style: linkBtnStyle }, selectedArchived.size === archivedDiscoveries.length ? "Tout désélectionner" : "Tout sélectionner"),
                        selectedArchived.size > 0 && React.createElement("span", { style: { fontSize: 12.5, color: "rgba(239,231,214,0.6)" } },
                            selectedArchived.size,
                            " s\u00E9lectionn\u00E9e(s)")),
                    selectedArchived.size > 0 && (React.createElement("div", { style: { display: "flex", gap: 10 } },
                        ...
                        React.createElement("button", { className: "btn btn-danger btn-sm", onClick: () => setConfirmPermanentDelete(true) },
                            ...
                            selectedArchived.size,
                            ")")))),
                archivedDiscoveries.map((d) => (React.createElement(DiscoveryCard, { key: d.id, discovery: d, selected: selectedArchived.has(d.id), onToggleSelect: toggleSelectArchived, onUpdateNotes: onUpdateNotes })))))))),
        React.createElement(ConfirmDialog, { open: confirmPermanentDelete, title: `Supprimer définitivement ${selectedArchived.size} découverte(s) ?`, body: "Cette action est irr\u00E9versible.", onConfirm: confirmPermanentDeleteSelected, onCancel: () => setConfirmPermanentDelete(false) })));
}
...
/* ------------------------------ Bootstrap ---------------------------------
   ...
   ============================================================================ */
console.log("%cID'Home Création — build 2026-08-23-j", "color:#9C4420; font-weight:bold; font-size:13px;");
