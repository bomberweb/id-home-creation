...
export default function App() {
    ...
    async function deleteLeads(ids) {
        ...
        flashToast(ok ? `${ids.length} demande(s) supprimée(s)` : "Échec de la suppression — réessayez");
    }
    ...
    return (React.createElement("div", { className: "ihc" },
        ...
        view === "admin" && (React.createElement(AdminDashboard, { config: config, setConfig: persistConfig, leads: leads, updateLeadStatus: updateLeadStatus, updateLeadNotes: updateLeadNotes, deleteLeads: deleteLeads, metrics: metrics, setPasswordHash: persistPasswordHash, onExit: () => setView("home"), flashToast: flashToast })),
...
/* ================================ ADMIN DASHBOARD ============================ */
function AdminDashboard({ config, setConfig, leads, updateLeadStatus, updateLeadNotes, deleteLeads, metrics, setPasswordHash, onExit, flashToast }) {
    ...
    return (React.createElement("div", { className: "ihc-admin" },
        ...
        React.createElement("div", { className: "ihc-admin-shell" },
            ...
            tab === "leads" && React.createElement(AdminLeads, { leads: leads, onToggleStatus: updateLeadStatus, onUpdateNotes: updateLeadNotes, onDeleteLeads: deleteLeads, flashToast: flashToast }),
...
function AdminLeads({ leads, onToggleStatus, onUpdateNotes, onDeleteLeads, flashToast }) {
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 } },
            React.createElement("div", { style: { display: "flex", gap: 14, alignItems: "center" } },
                React.createElement("button", { type: "button", onClick: selected.size === leads.length ? selectNone : selectAll, style: { background: "none", border: "none", color: "rgba(239,231,214,0.7)", fontSize: 12.5, textDecoration: "underline", textUnderlineOffset: "3px", padding: 0 } }, selected.size === leads.length ? "Tout désélectionner" : "Tout sélectionner"),
                selected.size > 0 && React.createElement("span", { style: { fontSize: 12.5, color: "rgba(239,231,214,0.6)" } },
                    selected.size,
                    " s\u00E9lectionn\u00E9e(s)")),
            React.createElement("div", { style: { display: "flex", gap: 10 } },
                selected.size > 0 && (React.createElement("button", { className: "btn btn-danger btn-sm", onClick: () => setConfirmDelete(true) },
                    React.createElement(Trash2, { size: 14 }),
                    " Supprimer (",
                    selected.size,
                    ")")),
        ...
        leads.map((l) => (React.createElement(LeadCard, { key: l.id, lead: l, selected: selected.has(l.id), onToggleSelect: toggleSelect, onToggleStatus: onToggleStatus, onUpdateNotes: onUpdateNotes }))),
        React.createElement(ConfirmDialog, { open: confirmDelete, title: `Supprimer ${selected.size} demande(s) ?`, body: "Cette action est d\u00E9finitive et supprime aussi les notes internes associ\u00E9es.", onConfirm: confirmDeleteSelected, onCancel: () => setConfirmDelete(false) })));
}
...
/* ------------------------------ Bootstrap ---------------------------------
   ...
   ============================================================================ */
console.log("%cID'Home Création — build 2026-08-23-f", "color:#9C4420; font-weight:bold; font-size:13px;");
